...
logger.info("Download Sentiment Analyzer")
...
def main(): 
    ...
    checkpoint_file_name="funny_bone"
    ...
    try:
        for batch, position in stream.get_records_continuously(
            ...
            for record in batch:
                ...
                logger.debug(f"Is the joke funny? {json_data.get('joke', '')}")
                if is_funny(json_data.get('joke'),sia):
                    logger.info(f"Joke is funny! Saving for later: {json_data.get('joke')}")
                else: 
                    logger.info(f"Joke Was not funny 😩: {json_data.get('joke')}")
...
def is_funny(joke:str,sentiment_analyzer, threshold=0.2):
    scores = sentiment_analyzer.polarity_scores(joke)
    logger.debug(scores)
    is_funny = scores['compound'] > threshold and scores['pos'] > 0
    return is_funny
